...
@pt.Register
class custom_fully_connected_with_l1_loss(pt.VarStoreMethod):
    def __call__(self, input_layer, output_size, l1_loss=True, l1_reg_val=0, scope=None, in_dim=None, stddev=0.02, bias_start=0.0):
        shape = input_layer.shape
        input_ = input_layer.tensor
        try:
            if(len(shape) == 4):
            ...
            with tf.variable_scope(scope or "Linear_with_L1"):
                matrix = tf.get_variable("Matrix", [in_dim or shape[1], output_size], dtype=tf.float32,
                                         initializer=tf.random_normal_initializer(stddev=stddev),
                                         regularizer=tf.contrib.layers.l1_regularizer(l1_reg_val))
                bias = self.variable("bias", [output_size], init=tf.constant_initializer(bias_start))
                return input_layer.with_tensor(tf.matmul(input_, matrix) + bias, parameters=self.vars)
        except Exception:
            import ipdb; ipdb.set_trace()
